...
module.exports = (ip, port, isDocker) => {
  ...
  const printChars = (count, char) => new Array(Math.max(0, count)).fill(char).join('');
  ...
  if (isDocker) {
    ...
    msg = `${chars.BLUE}${stars(91)}${chars.BR}${chars.RESET}`
      + `${chars.CYAN}Welcome to Shipyard! 🚀${chars.RESET}${chars.BR}`
    ...
    msg = `${chars.GREEN}┏${line(75)}┓${chars.BR}`
      + `┃ ${chars.CYAN}Welcome to Shipyard! 🚀${blanks(54)}${chars.GREEN}┃${chars.BR}`
